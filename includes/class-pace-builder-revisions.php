...
/**
 * Creates Revision for Pace Builder data whenever a post is created/updated
 *
 * @since      1.0.0
 * @package    PTPB
...
if ( ! class_exists( 'PTPB_Revisions' ) ) :

	/**
	 * Setup Revision support for PaceBuilder pages
	 * When a PaceBuilder page is reverted to a revision, the PaceBuilder shows the correct data
	 */
	class PTPB_Revisions {
		 ...
		 * @param $new_autosave
		 *
		 * @internal param object $Post $new_autosave The new post being autosaved.
		 */
...
if( ptpb_get_setting( 'revisions'  ) ) :
	...
	if ( version_compare( $wp_version, '4.1', '>=' ) ) {
		add_filter( 'wp_post_revision_meta_keys', 'ptpb_revision_metakeys' );
	}
	 ...
	 * @return array
	 */
	function ptpb_revision_metakeys( $meta_keys ) {
		$meta_keys[] = '_ptpb_sections';
